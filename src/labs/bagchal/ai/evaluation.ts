...
export class PositionEvaluator {
	 ...
	 */
	static evaluatePosition(state: GameState, adjacency: Map<number, number[]>): number {
		// Terminal states
	...
	private static evaluateMobility(state: GameState, adjacency: Map<number, number[]>): number {
		...
		const tigerWeight =
			state.phase === 'PLACEMENT'
				? PositionEvaluator.WEIGHTS.TIGER_MOBILITY / 2
				: PositionEvaluator.WEIGHTS.TIGER_MOBILITY;
		...
		for (let i = 0; i < state.board.length; i++) {
			...
			if (piece === 'TIGER') {
				...
				const captureOpportunities = PositionEvaluator.countCaptureOpportunities(
					i,
					state,
					adjacency
				);
		...
		return tigerMoves * tigerWeight + goatMoves * PositionEvaluator.WEIGHTS.GOAT_MOBILITY;
	}
	...
	// Removed unused method: getDirection
	// This was a helper method for calculating direction vectors that was never called
}
