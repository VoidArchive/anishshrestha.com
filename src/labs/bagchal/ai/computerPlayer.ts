import type { GameState, GameMode } from '$labs/bagchal/rules';
import type { Move } from './types';
import { Minimax } from '$core/ai';
import { BagchalEngine } from '$labs/bagchal/engine';
...
export class ComputerPlayer {
	...
	private hashGameState(state: GameState): string {
		...
		const boardStr = state.board
			.map((cell) => (cell === null ? '0' : cell === 'GOAT' ? '1' : '2'))
			.join('');
	...
	getBestMove(state: GameState): Move | null {
		try {
			...
			return bestMove || this.getEmergencyMove(state, ADJACENCY_MAP);
		} catch (err) {
