<script lang="ts">
	import type { Point } from '$labs/bagchal/rules';
...
{#if showAnimation && currentMove}
	<defs>
		...
		<linearGradient id="moveTrail" x1="0%" y1="0%" x2="100%" y2="0%">
			...
			<stop
				offset={`${Math.min(animationProgress * 100 + 20, 100)}%`}
				style="stop-color:#c92a2a;stop-opacity:0"
			/>
		</linearGradient>
	...
	{#if isPlacementMove()}
		...
		{#if toPoint}
			...
			<g transform={`translate(${toPoint.x}, ${toPoint.y})`}>
				...
				<!-- Center dot -->
				<circle cx="0" cy="0" r="3" fill="#c92a2a" opacity={animationProgress} />
			</g>
		{/if}
