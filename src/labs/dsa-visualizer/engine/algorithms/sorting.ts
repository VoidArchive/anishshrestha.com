...
export class SortingAlgorithms {
  ...
  private static quickSortWrapper(arr: number[]): AnimationStep[] {
    const steps = this.quickSort(arr, 0, arr.length - 1);
    const n = arr.length;
  ...
  private static quickSort(arr: number[], low: number, high: number): AnimationStep[] {
    ...
    if (low < high) {
      ...
      steps.push({
        ...
        state: { 
          comparing: []
          // Note: We'll accumulate sorted elements in the final completion step
        },
  ...
  private static partition(arr: number[], low: number, high: number): { pivotIndex: number; partitionSteps: AnimationStep[] } {
    const steps: AnimationStep[] = [];
  ...
  private static mergeSort(arr: number[], left: number, right: number): AnimationStep[] {
    ...
    if (left < right) {
      ...
      if (left === 0 && right === arr.length - 1) {
        ...
        steps.push({
          move: { type: 'HIGHLIGHT', indices: Array.from({ length: right - left + 1 }, (_, i) => left + i) },
  ...
  private static heapify(arr: number[], n: number, i: number): AnimationStep[] {
    const steps: AnimationStep[] = [];
    let largest = i;
    const left = 2 * i + 1;
    const right = 2 * i + 2;
