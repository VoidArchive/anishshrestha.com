<script lang="ts">
	...
	let { gameState, isComputerThinking = false, isPlayingComputer = false, playerSide = 'GOAT' }: Props = $props();
...
<div class="section-card">
	...
	<div class="space-y-3">
		...
		{#if isComputerThinking && isComputerTurn && !gameState.winner}
			<div class="flex items-center gap-2 p-2 bg-bg-primary rounded border border-primary-red/20">
				<div class="thinking-dots">
					<span></span>
					<span></span>
					<span></span>
				</div>
				<span class="text-sm text-primary-red font-medium">AI is thinking...</span>
			</div>
		{/if}
