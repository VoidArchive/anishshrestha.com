<script lang="ts">
	import { gameState, resetGameState } from '$games/bagchal/store.svelte';
	import type { PieceType, Player, GamePhase } from '$games/bagchal/rules';
	...
	function undoMove(undoCount: number = 1) {
		for (let i = 0; i < undoCount && gameStateHistory.length > 0; i++) {
			...
			if (previousState) {
				...
				if (i === undoCount - 1) { // Only update move history on the last undo
