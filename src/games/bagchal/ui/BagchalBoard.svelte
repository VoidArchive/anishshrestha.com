			...
			if (jumpedPoint) {
				...
				const firstLine = lines.find(
					(line) =>
						(line.x1 === fromPoint.x &&
							line.y1 === fromPoint.y &&
							line.x2 === jumpedPoint.x &&
							line.y2 === jumpedPoint.y) ||
						(line.x2 === fromPoint.x &&
							line.y2 === fromPoint.y &&
							line.x1 === jumpedPoint.x &&
							line.y1 === jumpedPoint.y)
				);
...
<svg width="100%" viewBox="0 0 500 500" class="mx-auto block">
	...
	{#each lines as line}
		{@const isValidLine = validLines.some(
			(vl) => vl.x1 === line.x1 && vl.y1 === line.y1 && vl.x2 === line.x2 && vl.y2 === line.y2
		)}
		{@const isAiAnimationLine =
			showAiAnimation &&
			aiAnimationPath.lines.some(
	...
	{#each points as { id, x, y }}
		...
		{#if piece === 'TIGER'}
			<g transform={`translate(${x}, ${y})`} class="piece-group">
				...
				{#if selected}
					<circle cx={0} cy={0} r="4%" fill="url(#tigerGlow)" class="selection-ring" />
					<circle
						cx={0}
						cy={0}
						r="3.5%"
				...
				<image
					...
					x="-3.5%"
					y="-3.5%"
					class="piece-text cursor-pointer select-none {selected
						? 'piece-selected'
						: 'piece-hover'}"
		...
		{:else if piece === 'GOAT'}
			<g transform={`translate(${x}, ${y})`} class="piece-group">
				...
				{#if selected}
					<circle cx={0} cy={0} r="4%" fill="url(#goatGlow)" class="selection-ring" />
					<circle
						cx={0}
						cy={0}
						r="3.5%"
		...
		{:else}
			...
			<g transform={`translate(${x}, ${y})`}>
				{#if valid}
					...
					<circle cx={0} cy={0} r="2%" fill="url(#validMoveGlow)" class="valid-move-glow" />
					<circle
						cx={0}
						cy={0}
						r="1.5%"
